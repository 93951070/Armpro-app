 ...
 *
 * <p>APK Signature Scheme v3 is a whole-file signature scheme which aims to protect every single
 * bit of the APK, as opposed to the JAR Signature Scheme which protects only the names and
 * uncompressed contents of ZIP entries. It is structurally similar to v2 but adds support for
 * SDK version targeting via additional attributes in the signed data.
 *
...
public abstract class V3SchemeSigner {
    /*
     * The two main goals of APK Signature Scheme v3 are:
    ...
    private static final int APK_SIGNATURE_SCHEME_V3_BLOCK_ID = 0x0f1c7b1a;

    /** Additional attribute ID for the minimum SDK version. */
    private static final int V3_ADDITIONAL_ATTR_MIN_SDK_VERSION_ID = 0x01;

    /** Additional attribute ID for the maximum SDK version. */
    private static final int V3_ADDITIONAL_ATTR_MAX_SDK_VERSION_ID = 0x02;
    ...
    public static class SignerConfig {
        ...
        /**
         * Minimum API Level on which this signature is valid.
         */
        public int minSdkVersion;

        /**
         * Maximum API Level on which this signature is valid. A value of {@link Integer#MAX_VALUE}
    ...
    /**
     * Signs the provided APK using APK Signature Scheme v3 and returns the APK Signing Block
    ...
    private static byte[] generateApkSigningBlock(
        ...
        byte[] apkSignatureSchemeV3Block =
                generateApkSignatureSchemeV3Block(signerConfigs, contentDigests);
        return generateApkSigningBlock(apkSignatureSchemeV3Block);
    }

    private static byte[] generateApkSigningBlock(byte[] apkSignatureSchemeV3Block) {
        // FORMAT:
        ...
        int resultSize =
                8 // size
                + 8 + 4 + apkSignatureSchemeV3Block.length // v3Block as ID-value pair
                + 8 // size
                + 16 // magic
                ;
        ...
        long pairSizeFieldValue = 4 + apkSignatureSchemeV3Block.length;
        result.putLong(pairSizeFieldValue);
        result.putInt(APK_SIGNATURE_SCHEME_V3_BLOCK_ID);
        result.put(apkSignatureSchemeV3Block);
    ...
    private static byte[] generateApkSignatureSchemeV3Block(
            List<SignerConfig> signerConfigs,
            Map<ContentDigestAlgorithm, byte[]> contentDigests)
                    throws NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        // FORMAT:
    ...
    private static byte[] generateSignerBlock(
        ...
        // Build additional attributes for V3: minSdkVersion and maxSdkVersion.
        List<Pair<Integer, byte[]>> additionalAttributes = new ArrayList<>(2);

        // minSdkVersion additional attribute
        byte[] minSdkVersionBytes = new byte[4];
        setUnsignedInt32LittleEndian(signerConfig.minSdkVersion, minSdkVersionBytes, 0);
        additionalAttributes.add(
                Pair.of(V3_ADDITIONAL_ATTR_MIN_SDK_VERSION_ID, minSdkVersionBytes));

        // maxSdkVersion additional attribute
        byte[] maxSdkVersionBytes = new byte[4];
        setUnsignedInt32LittleEndian(signerConfig.maxSdkVersion, maxSdkVersionBytes, 0);
        additionalAttributes.add(
                Pair.of(V3_ADDITIONAL_ATTR_MAX_SDK_VERSION_ID, maxSdkVersionBytes));
        signedData.additionalAttributes = additionalAttributes;
        ...
        signer.signedData = encodeAsSequenceOfLengthPrefixedElements(new byte[][] {
            ...
            // additional attributes (V3: minSdkVersion and maxSdkVersion)
    ...
    private static final class V3SignatureSchemeBlock {
        ...
        private static final class SignedData {
            ...
            public List<Pair<Integer, byte[]>> additionalAttributes;
        }
    }
